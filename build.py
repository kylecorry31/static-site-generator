...
config = read_config()
source = config["source"] if "source" in config else "src"
...
for root, dirs, files in os.walk(source):
    parent = root.replace("\\", "/").replace(f"{source}/", "").replace(source, "")
    if parent != "":
        site_data['collections'][parent] = get_collection_metadata(source, root)
